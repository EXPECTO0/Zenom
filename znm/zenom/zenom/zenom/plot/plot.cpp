...
Plot::Plot(QWidget* pParent): QwtPlot(pParent)
{
    ...
    QwtPlotPanner* plotPanner = new QwtPlotPanner(qobject_cast<QwtPlotCanvas*>(canvas()));
    connect(plotPanner, SIGNAL(panned(int,int)), this, SLOT(zoommed_or_panned()));

    // zoom in/out with the wheel
    PlotMagnifier* plotMagnifier = new PlotMagnifier(qobject_cast<QwtPlotCanvas*>(canvas()));
