...
void RadialGaugeSettingWidget::initGauge( AbstractGauge* pGauge )
{
    ...
    if ( radialGauge )
    {
        ...
        // Scale
        radialGauge->setLowerBound( ui->rangeMin->value() );
        radialGauge->setUpperBound( ui->rangeMax->value() );
        radialGauge->setOrigin( ui->startAngle->value() );
        radialGauge->setScaleArc( 0.0 , ui->sweepAngle->value() );

        // Ticks
        radialGauge->setScaleStepSize( ui->majorTick->value() );
...
void RadialGaugeSettingWidget::initFromGauge( AbstractGauge* pGauge )
{
    ...
    if ( radialGauge )
    {
        ...
        // Scale
        ui->rangeMin->setValue( radialGauge->lowerBound() );
        ui->rangeMax->setValue( radialGauge->upperBound() );
        ui->startAngle->setValue( radialGauge->origin() );
        ui->sweepAngle->setValue( radialGauge->maxScaleArc() );

        // Ticks
        ui->majorTick->setValue( radialGauge->scaleStepSize() );
