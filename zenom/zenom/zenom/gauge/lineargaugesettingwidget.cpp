    ...
    ui(new Ui::LinearGaugeSettingWidget)
{
    ...
    ui->scalePosition->setItemData ( 1, QwtThermo::TrailingScale );
    ui->scalePosition->setItemData ( 2, QwtThermo::LeadingScale );
...
void LinearGaugeSettingWidget::initGauge(AbstractGauge* pGauge)
{
    ...
    if ( linearGauge )
    {
        ...
        QwtThermo::ScalePosition scalePosition = (QwtThermo::ScalePosition)ui->scalePosition->itemData( index ).toInt();
        linearGauge->setOrientation( orientation, scalePosition );
        linearGauge->setLowerBound( ui->rangeMin->value() );
        linearGauge->setUpperBound( ui->rangeMax->value() );

        // Ticks
        linearGauge->setScaleStepSize(ui->majorTick->value());
...
void LinearGaugeSettingWidget::initFromGauge(AbstractGauge* pGauge)
{
    ...
    if ( linearGauge )
    {
        ...
        ui->rangeMin->setValue( linearGauge->minimum() );
        ui->rangeMax->setValue( linearGauge->maximum() );

        // Ticks
        ui->majorTick->setValue( linearGauge->scaleStepSize() );
